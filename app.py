...
FORMATO = "%H:%M"
MAX_LIMITE = "23:45"
GIF_URL = "https://media1.tenor.com/m/YA1b5_pPVS8AAAAd/ma%C3%A8del-mestiere-questa-checco-zalone.gif"
...
st.title("🕒 A che ora usciamo?")

# GIF sempre visibile
st.image(GIF_URL, use_container_width=True)

st.caption(f"Limite massimo slot straordinario fisso: **{MAX_LIMITE}**")

with st.sidebar:
    st.header("Parametri di lavoro")
    ore_giornaliere = st.number_input("Ore giornaliere", min_value=0, max_value=24, value=7)
    minuti_giornali = st.number_input("Minuti giornalieri", min_value=0, max_value=59, value=36)

st.markdown("<div class='card'>", unsafe_allow_html=True)
st.subheader("Orari (inserimento manuale HH:MM)")

c1, c2, c3 = st.columns(3)
ingresso_str = c1.text_input("Ingresso", value="09:00", placeholder="es. 09:03")
uscita_pranzo_str = c2.text_input("Uscita pranzo", value="13:00", placeholder="es. 13:02")
rientro_pranzo_str = c3.text_input("Rientro pranzo", value="14:00", placeholder="es. 14:01")

st.markdown("<div class='muted'>Puoi inserire qualsiasi minuto.</div>", unsafe_allow_html=True)
...
if st.button("Calcola", type="primary", use_container_width=True):
    try:
        ...
        if pausa_effettiva.total_seconds() < 0:
            raise ValueError("La pausa risulta negativa: verifica uscita e rientro pranzo.")
        ...
        while True:
            ...
            if corrente > limite_superiore:
                break
            rows.append({
                "Straordinario": f"+{k*15} min",
                "Orario": corrente.strftime(FORMATO)
            })
            k += 1

        st.subheader("Slot straordinario ogni 15 minuti")
